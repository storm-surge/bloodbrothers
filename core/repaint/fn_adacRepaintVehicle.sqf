...
if(life_bargeld < _basePrice) exitWith {hint "Du hast keine $5000 um den Lackierer zu bezahlen."};
...
if ((getPlayerUID player) != _vehOwner) exitWith {hint "Du bist nicht der Besitzer des Fahrzeugs das lackiert werden soll."};
		...
		while{true} do
		{
			...
			sleep 0.29;
			//[[player, "spraycan",8],"life_fnc_playSound",true,false] spawn life_fnc_MP; // wo,was,bis wieviel abstand
		...
		life_bargeld = life_bargeld - _basePrice;
		//Send toDB
		...
		//[[_veh,_color_index],"life_fnc_adaccolorVehicle",true,false] spawn life_fnc_MP;
