...
if(isNil "_vehicle" OR isNull _vehicle OR !(_vehicle getVariable "ateamLights")) exitWith {};
...
switch (typeOf _vehicle) do
{
	case "C_Offroad_01_F":
	{
	//[x, y, z] (Position3D) y senkrecht, x waagrecht
...
{  
	if(!(_vehicle getVariable "ateamLights")) exitWith {};
	if(_leftRed) then  
	{  
		_leftRed = false;  
