...
*/
private["_opfor"];
_opfor = [_this,0,Objnull,[Objnull]] call BIS_fnc_param;
...
[] spawn
{
	...
	while {true} do
	{
		...
		if(!([east,getPos player,30] call life_fnc_nearUnits) && (player getVariable["restrained",FALSE]) && vehicle player == player) exitWith {
...
titleText[format["Du wurdest von %1 gefesselt.",name _opfor],"PLAIN"];
