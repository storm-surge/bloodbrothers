...
switch (_code) do
{
	...
	{	
		...
		if (animationState player != "AovrPercMrunSrasWrflDf" && {isTouchingGround player} && {stance player == "STAND"} && {speed player > 2} && {!life_is_arrested} && {(velocity player) select 2 < 2.5} && {time - jumpActionTime > 1.5}) then {
...
case 79:
{
if((player getVariable "restrained") || (player getVariable "Escorting") || (player getVariable "transporting") || (life_is_arrested) || life_istazed) exitWith {hint "Das geht jetzt nicht.";};
...
if((player getVariable "restrained") || (player getVariable "Escorting") || (player getVariable "transporting") || (life_is_arrested) || life_istazed) exitWith {hint "Das geht jetzt nicht.";};
	...
	case 19:
	{
		...
		if(_shift && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && alive cursorTarget && cursorTarget distance player < 3.5 && isPlayer cursorTarget && cursorTarget getVariable "surrender" && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained") && speed cursorTarget < 1) then
		{
			[] call life_fnc_restrainActionCiv;
		};
		////////////////////////////////////////////////////////////////////////////////////////
		// OPFOR - > CIVS
		if(_shift) then {_handled = true;};
		if(_shift && playerSide == east && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && alive cursorTarget && cursorTarget distance player < 3.5 && isPlayer cursorTarget && animationState cursorTarget == "Incapacitated" && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained") && speed cursorTarget < 1) then
		{
			[] call life_fnc_restrainActionOpfor;
		};
		if(_shift && playerSide == east && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && alive cursorTarget && cursorTarget distance player < 3.5 && isPlayer cursorTarget && cursorTarget getVariable "surrender" && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained") && speed cursorTarget < 1) then
		{
			[] call life_fnc_restrainActionOpfor;
		};
		////////////////////////////////////////////////////////////////////////////////////////
		// Civs - > Cops
		if(_shift) then {_handled = true;};
		if(_shift && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && (side cursorTarget == west) && alive cursorTarget && cursorTarget distance player < 3.5 && isPlayer cursorTarget && cursorTarget getVariable "surrender" && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained") && speed cursorTarget < 1) then
		{
			[] call life_fnc_restrainActionCiv;
		};
	...
	case 34:
	{
		if ((player getVariable ["inSafezone", true]) && (playerside != east)) exitWith {hint "Das geht in der Safezone nicht.";};
		if (life_down22) exitWith {hint "Das geht jetzt nicht.";};
		if(_shift) then {_handled = true;};
		if(_shift && playerSide == civilian OR playerSide == east  && !isNull cursorTarget && cursorTarget isKindOf "Man" && isPlayer cursorTarget && alive cursorTarget && cursorTarget distance player < 4 && speed cursorTarget < 1) then
		{
	...
	case 38: 
	{
		...
		if(_shift && playerSide == east) then {
			if(vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_F"]) then{
				if(!isNil {vehicle player getVariable "ateamlights"}) then {
					[vehicle player] call life_fnc_opforLights;
					_handled = true;
				};
			};
		};
