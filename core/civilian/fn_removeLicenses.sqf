...
switch (_state) do
{
	...
	case 2:
	{
		if(license_civ_driver OR license_civ_air OR license_civ_truck OR license_civ_boat) then {
			...
			hint "Du hast wegen Fahrlaessiger-Toetung alle deine Fahrzeug-Scheine verloren.";
		};
	};
	...
	case 4:
	{
		if(license_civ_driver OR license_civ_truck) then {
			...
			cutText ["","Du hast alle deine Führerscheine wegen zu schnellem fahren verloren!"];
		};
	};
};