...
if((_price * _amount) > life_bargeld && {!isNil "_hideout" && {!isNil {grpPlayer getVariable "gang_bank"}} && {(grpPlayer getVariable "gang_bank") <= _price * _amount}}) exitWith {hint "You don't have that much money!"};
...
if(([true,_type,_amount] call life_fnc_handleInv)) then
{
 ...
 [life_bargeld] call life_fnc_numberText
 ],
 ...
 if((_price * _amount) > life_bargeld) exitWith {[false,_type,_amount] call life_fnc_handleInv; hint "You don't have that much money!";};
 hint format["You bought %1 %2 for $%3",_amount,_name,[(_price * _amount)] call life_fnc_numberText];
 __SUB__(life_bargeld,_price);
 };
 } else {
 if((_price * _amount) > life_bargeld) exitWith {hint "You do not have enough money!"; [false,_type,_amount] call life_fnc_handleInv;};
 hint format["You bought %1 %2 for $%3",_amount,_name,[(_price * _amount)] call life_fnc_numberText];
 __SUB__(life_bargeld,(_price * _amount));
 };
