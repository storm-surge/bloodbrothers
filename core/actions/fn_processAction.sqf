...
_itemInfo = switch (_type) do
{
	...
	case "gold": {["gold","goldr",1350,"Verarbeite Gold"]};
	case "silver": {["silver","silverr",1050,"Verarbeite Silber"]};
	case "coal": {["coal","coalr",350,"Verarbeite Kohle"]};
	...
	case "grapes": {["grapes","vodka",250,"Processing Grapes into Vodka"]};
...
{
	if(life_bargeld < _cost) exitWith {hint format["You need $%1 to process without a license!",[_cost] call life_fnc_numberText]; 5 cutText ["","PLAIN"]; life_is_processing = false;};
	...
	life_bargeld = life_bargeld - _cost;
	life_is_processing = false;
};	