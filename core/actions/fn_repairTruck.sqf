...
if(life_action_repair) exitWith {};
_veh = cursorTarget;
...
if((_veh isKindOf "Car") OR (_veh isKindOf "Ship") OR (_veh isKindOf "Air")) then
{
	if("ToolKit" in (items player)) then
	{
		...
		_cP = 0.01;
		life_action_repair = true;
		[player, _veh] call life_fnc_repairEquipment;
		//InBaseMoves_repairVehiclePne - AinvPknlMstpSnonWnonDnon_medic_1
		while{true} do
		{
			if(animationState player != "InBaseMoves_repairVehiclePne") then {
				[[player,"InBaseMoves_repairVehiclePne"],"life_fnc_animSync",true,false] spawn life_fnc_MP;
